...
FastAPI + MySQL vector store + sentence-transformers + Claude
"""
...
class GenerateTestRequest(BaseModel):
    ...
    course_id: Optional[int] = None  # ✅ Now actually used
...
def _ingest_lecture_task(lecture_id: int):
    try:
        ...
        print(f"[INGEST] ❌ Lecture {lecture_id} failed: {e}")
...
async def ingest_course_endpoint(
    ...
    def task():
        try:
            ...
            print(f"[INGEST] ❌ Course {req.course_id} failed: {e}")
...
async def generate_test(req: GenerateTestRequest):
    ...
    # ✅ FIX: Pass BOTH lecture_id AND course_id so retrieval is scoped correctly
    context, sources = retrieve_context(
        query=req.topic,
        lecture_id=req.lecture_id,
        course_id=req.course_id,   # ← was missing before — caused global search = bad questions
    )
    ...
    if not context:
        raise HTTPException(
            ...
            detail=(
                ...
                "Make sure the lecture/course has been ingested via /api/ingest/lecture or /api/ingest/course."
            )
        )
    ...
    test_data["course_id"]     = req.course_id
    test_data["student_id"]    = req.student_id
    test_data["topic"]         = req.topic
    test_data["difficulty"]    = req.difficulty
    return test_data
